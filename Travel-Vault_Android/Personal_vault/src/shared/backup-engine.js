...
import { getCachedFinanceData, setCachedFinanceData } from './db.js';
...
export async function exportEncryptedBackup(databaseName, preloadedData = null) {
  ...
  const data = preloadedData || await getCachedFinanceData();
  ...
  try {
    ...
    const fileName = `Vault_${new Date().getTime()}.vaultbox`;
  ...
  input.onchange = async (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        if (parsedData && (parsedData.transactions || parsedData.categories)) {
          await setCachedFinanceData(parsedData);
