...
export async function renderDashboard(container) {
  ...
  function buildStatusCard(member, trips, documents, year, familyDefaults) {
    ...
    if (!members.length) {
      content.innerHTML = `
        <div class="empty-state">
          <div class="empty-state-icon">👨‍👩‍👧‍👦</div>
          <div class="empty-state-title">No family members yet</div>
          <div class="empty-state-text">Add members in Settings to get started</div>
          <button class="btn btn-primary" style="margin-top:20px;" onclick="">Go to Settings</button>
        </div>`;
      content.querySelector('.btn')?.addEventListener('click', () => navigate('settings'));
      return;
    }
    ...
    if (!visibleMembers.length) {
      content.innerHTML = `
        <div class="empty-state">
          ...
          <div class="empty-state-text">No family members match the active filters</div>
        </div>`;
      return;
    }

    // Summary row: how many currently in Qatar
    const inQatar = members.filter(m => getCurrentLocation(m, trips).location === 'Qatar').length;
    const totalDangerDocs = documents.filter(d => {
      const st = expiryStatus(d.expiryDate);
      return st === 'danger' || st === 'expired';
    }).length;

    content.innerHTML = `
      ${totalDangerDocs > 0 ? `
        <div style="background:var(--danger-bg);border:1px solid #FECACA;border-radius:var(--radius-md);padding:12px 16px;display:flex;align-items:center;gap:10px;">
          <span style="font-size:20px;">🚨</span>
          <div>
            <div style="font-size:14px;font-weight:700;color:#991B1B;">${totalDangerDocs} document${totalDangerDocs > 1 ? 's' : ''} need attention</div>
            <div style="font-size:12px;color:#B91C1C;">Expiring within 30 days or already expired</div>
          </div>
          <button onclick="" class="btn btn-danger" style="padding:6px 12px;font-size:12px;margin-left:auto;" id="view-docs-btn">View</button>
        </div>
      ` : ''}
    ...
    const cardsContainer = document.getElementById('member-cards');
    cardsContainer.innerHTML = visibleMembers.map(member => {
      const { location, days } = getCurrentLocation(member, trips);
      const memberDocs = getPersonDocs(member, documents);
      const nextExpiry = getNextExpiry(memberDocs);
      const yearDays   = getYearlyQatarDays(member.id, trips, year);

      // Apply doc status filter per member
      if (filterDocStatus !== 'all') {
        const hasMatchingDoc = memberDocs.some(d => {
          const st = expiryStatus(d.expiryDate);
          if (filterDocStatus === 'expiring') return st === 'warning' || st === 'danger';
          if (filterDocStatus === 'expired')  return st === 'expired';
          return true;
        });
        if (!hasMatchingDoc) return '';
      }
      ...
      if (nextExpiry) {
        ...
        expiryHtml = `
          <div style="font-size:12px;color:var(--text-muted);margin-top:8px;padding-top:8px;border-top:1px solid var(--border-light);">
      ...
      return `
        <div class="person-card fade-in" data-member-id="${member.id}" style="cursor:pointer;">
          <div style="display:flex;align-items:center;gap:12px;">
            <div class="person-avatar" style="background:${member.color || '#EEF2FF'};">
              ${member.emoji || '👤'}
            </div>
            <div style="flex:1;min-width:0;">
              <div style="font-size:16px;font-weight:700;color:var(--text);">${member.name}</div>
    ...
    // Tap member card → travel log filtered to that person
    cardsContainer.querySelectorAll('.person-card').forEach(card => {
      card.addEventListener('click', () => {
        navigate('travel-log', { personId: card.dataset.memberId });
      });
    });
  }
